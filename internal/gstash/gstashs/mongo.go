...
var mongoGstashPrefix = "PMP_MONGO"
...
func (m *MongoGstash) Init(config interface{}) error {
	...
	overrideErr := envconfig.Process(mongoGstashPrefix, m.dbConf)
	if overrideErr != nil {
		log.Errorf("Failed to process environment variables for mongo gstash: %s", overrideErr.Error())
	}
...
func (m *MongoGstash) capCollection() (ok bool) {
	...
	if strconv.IntSize < 64 {
		log.Warn("Gstash running < 64bit architecture. Not capping collection as max size would be 2gb")
...
func (m *MongoGstash) collectionExists(name string) (bool, error) {
	sess := m.dbSession.Copy()
	defer sess.Close()
...
func (m *MongoGstash) ensureIndexes() error {
	var err error
